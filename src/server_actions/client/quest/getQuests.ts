...
import { getQuests } from "../../definitions/quest/quest_actions";
import { getQuestDetails } from "../../definitions/quest/quest_actions";
...
export const getGetQuestsQueryKey = (variantName: "getQuestDetails" | null, ...args: any[]) => {
  return [variantName ? ({ "getQuestDetails": "getQuests.getQuestDetails" })[variantName] : "getQuests", ...args] as const;
};
export const getGetQuestsQueryOptions = (variantName: "getQuestDetails" | null, ...args: any[]) => {
  return {
    queryKey: getGetQuestsQueryKey(variantName, ...args),
    queryFn: async () => { return variantName === "getQuestDetails" ? getQuestDetails(...args as [string]) : getQuests("read", ...args as []); },
  } as const;
};
export const prefetchGetQuests: PrefetchGetQuests = (variantName: "getQuestDetails" | null, ...args: unknown[]) =>
  (qc: QueryClient) =>
    qc.prefetchQuery(getGetQuestsQueryOptions(variantName, ...args));

export interface PrefetchGetQuests {
  (variantName: "getQuestDetails", ...args: [string]): (qc: QueryClient) => Promise<unknown>;
