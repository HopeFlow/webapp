...
export const redirectToChat = (props: { nodeId: string, questId: string }): never =>
  redirect(
    [
      "",
...
const routeSpecs: Map<
  "Sample" | "Home" | "Notifications" | "Profile" | "Trophies" | "Index" | "CreateAccount" | "Login" | "Quest" | "Link" | "Chat",
  {
    pathRegExp: RegExp;
...
> = new Map([
  ...
  ["Chat", { pathRegExp: /^\/chat\/(?<questId>[^/]+)\/(?<nodeId>[^/]+)$/, paramsTypeDef: z.object({ questId: z.string(), nodeId: z.string() }), searchParamsTypeDef: undefined, isPublic: false }],
]);
